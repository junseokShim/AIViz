...
### 고급 신호 분석 (Advanced Signal Analysis)

| 방법 | 설명 |
|------|------|
| **FFT** | 단일 진폭/전력 스펙트럼, 피크 감지 (기존 기능 개선) |
| **PSD (Welch)** | 평균화된 Welch 주기도: 낮은 분산의 PSD 추정 |
| **STFT** | 단시간 푸리에 변환: 스펙트로그램 + 시간별 지배 주파수 추적 |
| **CWT** | 연속 웨이블릿 변환: 다해상도 스칼로그램 (PyWavelets 선택 설치) |
| **S-Transform** | Stockwell Transform: 주파수 적응형 가우시안 창, 절대 위상 보존 |
| **Band Power** | 주파수 대역별 에너지 분석 (Welch 또는 FFT 기반) |
| **Envelope Spectrum** | Hilbert 변환 기반 포락선 스펙트럼 (베어링 결함 감지) |
| **Cepstrum** | 실수 케프스트럼: 하모닉 계열 및 기본 주파수 추정 |

### 데이터베이스 연동 (DB Integration)

| 기능 | 설명 |
|------|------|
| **SQLite** | 내장 sqlite3 기반, 설치 불필요, 항상 사용 가능 |
| **MySQL** | SQLAlchemy + pymysql (선택 설치) |
| **PostgreSQL** | SQLAlchemy + psycopg2 (선택 설치) |
| **테이블 브라우저** | 테이블 목록, 미리보기, 스키마 확인 |
| **SQL 쿼리** | 직접 SQL 실행, 결과를 AIViz 데이터셋으로 로드 |
| **가져오기** | DB 테이블 → AIViz 활성 데이터셋 |
| **내보내기** | AIViz 현재 데이터 → DB 테이블 (교체/추가 모드) |

### 아키텍처 개선

- `aiviz/analytics/signal/` – 신호 분석 전용 모듈 패키지 (UI 의존 없음)
- `aiviz/db/` – DB 레이어 (SQLite 백엔드 + SQLAlchemy 다중 DB)
- AI 인사이트: PSD, STFT, CWT, S-Transform, Band Power, Envelope 설명 추가 (한국어)

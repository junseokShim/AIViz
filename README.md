...
| **폴더 일괄 로드** | 폴더 내 CSV/Excel/JSON 파일 일괄 파싱, 스키마 불일치 처리, 결합/개별 모드 |
| **파생 컬럼 생성** | 수식 엔진으로 새 컬럼 생성 (`actLoad / command_RPM` 등), 모든 탭에서 즉시 사용 가능 |
| 차트 빌더 | 선형·산점도·막대·히스토그램·박스플롯·상관관계 히트맵 |
| 시계열 분석 | 롤링 통계, 추세 감지, 이상치 탐지, 스무딩 |
| 주파수 분석 | FFT, 진폭/전력 스펙트럼, 피크 감지, 밴드 에너지, 스펙트로그램 |
| **AC 성분 분석** | DC 오프셋 제거 후 AC 신호에 대한 FFT 및 RMS 분석, 비교 시각화 |
| 이미지 분석 | 픽셀 통계, 채널 히스토그램, 주요 색상, AI 설명 |
| 예측 모듈 | Holt-Winters / ARIMA / Simple ES (statsmodels 기반) |
| **클러스터링** | KMeans / DBSCAN, 클러스터 레이블 자동 추가, 실루엣 점수 |
| **신경망 (MLP)** | 간단한 다층 퍼셉트론 회귀/분류, 학습/테스트 분할, 지표 출력 |
| AI 어시스턴트 | Ollama 기반 데이터 요약, 차트 제안, 패턴 설명, 자유 대화 |
| **AI 인사이트 한국어** | 모든 AI 인사이트가 한국어로 출력, 복사/저장 버튼 지원 |
| 멀티모달 분석 | LLaVA를 통한 실제 이미지 픽셀 기반 AI 설명 |
| 보고서 내보내기 | PDF (fpdf2) 및 HTML 자동 생성 |
| 오프라인 폴백 | Ollama 비활성 시에도 모든 분석 기능 정상 동작 |
| **안전한 컬럼 접근** | 컬럼 누락 시 크래시 없이 경고 표시, 대소문자 및 공백 정규화 |
...
- **클러스터링/MLP**: scikit-learn이 필요합니다 (`pip install scikit-learn`)
- **파생 컬럼 유지**: 앱 재시작 시 파생 컬럼이 초기화됩니다 (세션 내에서만 유지)
...
- [ ] 밝은 테마(라이트 모드) 지원
- [ ] 파생 컬럼 세션 저장/불러오기
- [ ] 고역통과/대역통과 필터 (AC 분석 확장)
- [ ] PyTorch 기반 딥러닝 모듈 (현재 scikit-learn MLP)
